...
// esse código também inclui implementação de uma TABELA HASH com string
...
void inicializar_tabela(Pessoa t[]){ // inicializa tabela com string
    for(int i = 0 ; i<TAM ; i++)
        strcpy(t[i].nome, "");
}

int funcaoHashString(const char *str){
    int tam = strlen(str);
    unsigned int hash = 0;
    for(int i = 0 ; i<tam ; i++){
        hash += str[i] * (i+1);
...
Pessoa *busca(Pessoa vet[], const char *str){
    int id = funcaoHashString(str);
    printf("\nIndice gerada: %d\n", id);
    while(strlen(vet[id].nome)>0)
    {
        if(strcmp(vet[id].nome, str) == 0 )
           return &vet[id];
        else
           id = funcaoHash(id + 1);
    }
